...
contract AIRBNB {
    ...
    function addRental(
        ...
        require(msg.sender == owner, "Only the owner of the rental contract can put up rentals");
    ...
    function addDatesBooked(uint256 id, string[] memory newBookings) public payable {
        require(id < counter, "No such rental available");
        require(checkBookings(id, newBookings), "Already booked for the requested date");
        require(msg.value == (rentals[id].pricePerDay * 1 ether * newBookings.length), "Please submit the rental asking price in order to complete your purchase");
